...
function Project(props: ProjectProps): ReactElement {
    ...
    return (
        <li className="relative flex-col md:odd:flex-row-reverse md:flex-row mx-2 md:mx-6 max-w-[1175px] flex-wrap gap-2 md:gap-16 flex bg-white  md:bg-[#FFFFFF80] items-center p-6 z-10 backdrop-blur-md shadow-[0_0_12px_rgba(0,0,0,0.2)] rounded-md">
            <Fade cascade right={props.side} left={!props.side}>
                <div className="flex-[2] grid grid-cols-2 gap-2 items-stretch">
                    {image.map(img =>
